function [selectedImg R P AP] = VS_GCH_M(queryName, distMeasure, Q, PCA)

%% This function will use the images global colour histogram with moment descriptors and the specified distance measure to rank the database images according to the similarity to the specified query image.
...
DESCRIPTOR_SUBFOLDER='globalColourHistoMoment';
...
load(featFileQuery, 'F_GCH_M');
featFileQueryData=F_GCH_M;
...
for filenum=1:length(allfiles) % Iterate through array containing all the image files
    ...
    load(featfile,'F_GCH_M');
    ALLFILES{ctr}=imgfname_full;
    ALLFEAT=[ALLFEAT ; F_GCH_M];
