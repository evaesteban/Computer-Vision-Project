function [selectedImg R P AP] = VS_EOH(queryName, distMeasure, PCA)

%% This function will use the edge orientation histogram descriptors with the specified distance measure to rank the database images according to the similarity to the specified query image.
...
DESCRIPTOR_SUBFOLDER='edgeOrientHisto';
...
load(featFileQuery, 'F_EOH');
featFileQueryData=F_EOH;
...
if(PCA == true)
    ...
    sumEImg = 0;
    % Iterate through eigenvalues to determine the null space 
    for(i=1:size(e.val))
            if(sumEImg < sumE)
