function V=EOH(qimg, Nrows, Ncols, Q) % Is the image already divided by 255?
...
% Convert input RGB image to greyscale image
img = RGB_To_Gray(qimg);
...
% filter
verticEdgesImg = conv2(img, SobelX, 'same');
horizEdgesImg = conv2(img, SobelY, 'same');
...
%concatArray = reshape(concatArray,size(concatArray,1)*size(concatArray,2),1);
