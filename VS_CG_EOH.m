function [selectedImg R P AP] = VS_CG_EOH(queryName, distMeasure, PCA)

%% This function will call the function to compute the colour grid combined with edge orientation histogram descriptor for every image in the database. It 
%% will use the specified distance measure to rank the database images according to the similarity to the specified query image.
...
DESCRIPTOR_SUBFOLDER='colourGridEOH';
...
load(featFileQuery, 'F_CG_EOH');
featFileQueryData=F_CG_EOH;
...
for filenum=1:length(allfiles) % Iterate through array containing all the image files
    ...
    load(featfile,'F_CG_EOH');
    ALLFILES{ctr}=imgfname_full;
    ALLFEAT=[ALLFEAT ; F_CG_EOH];
...
if(PCA == true)
    ...
    sumE = sumE*0.81; % Percentage of eigenvalues
    sumEImg = 0;
...
for i=1:NIMG  
    ...
    elseif(distMeasure == 'M')
       ...
       thedst=Mahalanobis(featFileQueryData,candidate);
    end   
...
% Arrange results to display for first query
